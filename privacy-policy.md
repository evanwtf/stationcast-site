...
StationCast ("we," "us," "our," or "the App") is a personal weather station companion. This Privacy Policy explains what information we handle when you use the StationCast iOS app, the iOS widget extension, the Apple Watch app, the StationCast Android app, and the StationCast backend service that the apps talk to. It also covers the optional Amazon Alexa skill, when you choose to enable it.
...
- The StationCast app version, build number, commit short hash, platform (`ios` or `android`), and device class (for example, `phone`, `pad`, `tablet`, or `watch`).
